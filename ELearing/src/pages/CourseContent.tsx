...
import { ArrowLeft, Plus, PlayCircle, FileQuestion, Edit, Trash2, Search, X, Check, Eye, Clock, RefreshCw } from 'lucide-react';
...
const CourseContent = () => {
    ...
    const [videoKey, setVideoKey] = useState(0); // For refreshing video iframe

    // Flatten modules into a list of lessons for display
    ...
    // Check if we should auto-play a video from navigation state
    useEffect(() => {
        if (location.state?.autoPlayVideo) {
            const videoData = location.state.autoPlayVideo;
            setPreviewContent(videoData);
                                ...
                                <div className="space-y-3">
                                    ...
                                    {previewContent.videoLink && (
                                        <div className="flex justify-end">
                                            <button
                                                ...
                                                className="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-bold text-xs transition-all flex items-center gap-2 border border-gray-200"
                                                title="Refresh Video"
                                            >
                                    ...
                                    <div className="aspect-video bg-gray-900 rounded-xl overflow-hidden relative">
                                        {previewContent.videoLink ? (
                                            <iframe
                                                key={videoKey}
                                                src={getEmbedUrl(previewContent.videoLink)}
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <div className="text-center">
                                                    <PlayCircle className="w-20 h-20 text-white/80 mx-auto mb-3" />
                                                    <p className="text-white/60 text-sm font-medium">No video URL provided</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
