...
const CourseDetails = () => {
    ...
    useEffect(() => {
        const fetchCourse = async () => {
            if (!id) return;
            try {
                // Use api service to ensure authentication headers are included
                const response = await api.get(`/courses/${id}`);
                const data = response.data;
                // map based on potential backend response structure adjustments
                setCourse({
                    id: Number(data.id),
                    title: data.title,
                                                ...
                                                <div className="divide-y divide-gray-100">
                                                    {module.lessons.map((lesson) => (
                                                        <button
                                                            key={lesson.id}
                                                            onClick={() => {
                                                                if ((lesson.type === 'video' || lesson.type === 'VIDEO') && lesson.videoLink) {
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-4">
                                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${lesson.type === 'video' || lesson.type === 'VIDEO' ? 'bg-blue-50 text-[#0ea5e9]' : 'bg-purple-50 text-purple-600'
                                                                    }`}>
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="flex items-center gap-2 mt-0.5">
                                                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${lesson.type === 'video' || lesson.type === 'VIDEO'
                                                                            ? 'bg-blue-50 text-blue-600 border-blue-100'
                                                                            : 'bg-purple-50 text-purple-600 border-purple-100'
                                                                            } uppercase`}>
                                                                            {lesson.type}
                                                                        </span>
                                                                        <span className="text-xs text-gray-500">{lesson.duration || '0m'}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            {lesson.type === 'video' || lesson.type === 'VIDEO' ? (
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <option value={course.author}>{course.author}</option>
                                    {/* Instructors would be loaded here */}
                                </select>
                            </div>
