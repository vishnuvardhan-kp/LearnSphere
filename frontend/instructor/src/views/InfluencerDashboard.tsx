...
export const InfluencerDashboard = () => {
    ...
    const fetchCourses = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${API_URL}/courses`, getAuthHeader());
            // Transform backend data to frontend model
            const mappedCourses = res.data.map((c: any) => ({
                id: c.id,
                title: c.title,
                tags: c.tags || [],
                views: parseInt(c.enrolled_students) || 0, // Using enrollment as views for now
                totalLessons: parseInt(c.total_lessons) || 0,
                duration: c.total_duration ? `${c.total_duration}m` : '0m',
    ...
    const fetchStudents = async () => {
        try {
            const res = await axios.get(`${API_URL}/students`, getAuthHeader());
            // Map backend students to reporting format
            const mappedUsers = res.data.map((s: any) => ({
                id: s.user_id,
                name: s.name,
                status: s.status === 'completed' ? 'completed' : (s.progress > 0 ? 'active' : 'pending'),
                progress: s.progress,
                enrolledDate: new Date(s.enrolled_at).toLocaleDateString(),
                lastActive: s.last_accessed ? new Date(s.last_accessed).toLocaleDateString() : '-',
                timeSpent: '-' // Not tracked yet
            }));
            setReportingUsers(mappedUsers);
        } catch (err) {
    ...
    const confirmCreateCourse = async () => {
        ...
        try {
            ...
            await axios.post(`${API_URL}/courses`, {
                ...
                description: '',
                short_description: '',
                course_admin_id: user.id, // Explicitly passing ID
                visibility: 'EVERYONE',
                access_rule: 'OPEN',
                price: 0
            }, getAuthHeader());
    ...
    const saveCourse = async (updatedCourse: any) => {
        try {
            // Need to transform lessons if necessary, but backend might accept flat if we didn't implement modules fully yet.
            // Backend expects 'modules' array. I'll mock a default module for now.
            ...
            await axios.put(`${API_URL}/courses/${updatedCourse.id}`, {
                title: updatedCourse.title,
                description: updatedCourse.description,
                image_url: updatedCourse.image,
                price: updatedCourse.price,
                tags: updatedCourse.tags,
                modules: modules,
                // Also toggle publish status separately if changed? 
                // Using updateCourse for general fields.
                course_admin_id: profile?.id
            }, getAuthHeader());

            if (updatedCourse.status === 'published' && !updatedCourse.is_published) {
                await axios.patch(`${API_URL}/courses/${updatedCourse.id}/publish`, { is_published: true }, getAuthHeader());
            } else if (updatedCourse.status === 'draft' && updatedCourse.is_published) {
                await axios.patch(`${API_URL}/courses/${updatedCourse.id}/publish`, { is_published: false }, getAuthHeader());
            }
            ...
            {activeTab === 'courses' && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
                        {/* Search Input and View Toggle (kept same) */}
