...
import { Bot, Instagram, Youtube, Check, ChevronRight, ChevronLeft, Target, Users2, Sparkles } from 'lucide-react';
                    ...
                    {step === 1 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="text-center">
                                ...
                                <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Connect Your Platforms</h1>
                                <p className="text-gray-500 font-medium">Select the channels you want to protect with AI</p>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[
                                    { id: 'instagram', name: 'Instagram', icon: Instagram, color: 'hover:border-pink-500 hover:bg-pink-50' },
                                    { id: 'youtube', name: 'YouTube', icon: Youtube, color: 'hover:border-red-500 hover:bg-red-50' }
                                ].map(p => (
                                    <button
                                        key={p.id}
                                        onClick={() => togglePlatform(p.id)}
                                        className={`relative p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-4 ${platforms.includes(p.id) ? 'border-brand-blue bg-brand-blue/5' : 'border-gray-100 bg-white ' + p.color
                                            }`}
                                    >
                    ...
                    {step === 2 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="text-center">
                                ...
                                <h1 className="text-3xl font-extrabold text-gray-900 mb-2">What's Your Niche?</h1>
                                <p className="text-gray-500 font-medium">This helps our AI understand your content context</p>
                            </div>

                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {['Fashion', 'Technology', 'Lifestyle', 'Gaming', 'Fitness', 'Travel', 'Food', 'Business', 'Other'].map(n => (
                                    <button
                                        key={n}
                                        onClick={() => setNiche(n)}
                                        className={`p-4 rounded-2xl border-2 font-bold transition-all text-sm ${niche === n ? 'border-brand-blue bg-brand-blue/5 text-brand-blue' : 'border-gray-100 bg-white text-gray-500 hover:border-gray-200'
                                            }`}
                                    >
                                        {n}
                                    </button>
                    ...
                    {step === 3 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="text-center">
                                ...
                                <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Audience Demographics</h1>
