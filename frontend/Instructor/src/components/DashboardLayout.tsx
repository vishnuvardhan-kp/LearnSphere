...
import { Bot, Home, BarChart3, Users, Settings, LogOut, Bell, Search, ChevronRight, Wallet, Target } from 'lucide-react';
...
export const DashboardLayout = ({ children, role }: { children: React.ReactNode, role: 'company' | 'influencer' }) => {
    ...
    const navItems = [
        ...(role === 'company' ? [
            ...
            { label: 'AI Agent', icon: Bot, path: '/company/ai-agent' },
            { label: 'Funds', icon: Wallet, path: '/company/funds' },
            { label: 'Influencers', icon: Users, path: '/company/influencers' },
            { label: 'Analytics', icon: BarChart3, path: '/company/analytics' },
            { label: 'Campaigns', icon: Target, path: '/company/campaigns' },
            { label: 'Settings', icon: Settings, path: '/settings' },
        ] : [
            { label: 'Courses', icon: Bot, path: '/influencer-dashboard' },
            { label: 'Reporting', icon: BarChart3, path: '/influencer-dashboard?tab=reporting' },
            { label: 'Settings', icon: Settings, path: '/influencer-dashboard?tab=settings' }
        ]),
    ];
                ...
                <div className="p-6">
                    <Link to="/" className="flex items-center gap-3 group">
                        ...
                        <span className="text-xl font-black tracking-tighter text-gray-900">
                            Vyral<span className="text-brand-blue">AI</span>
                        </span>
                    </Link>
                </div>
